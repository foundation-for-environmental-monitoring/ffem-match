...
public class FinderPatternFinder {
  ...
  private static final int EXPECTED_QR_CORNER_COUNT = 4;
...
//    ResultPoint.orderBestPatterns(patternInfo);
  ...
  private boolean haveMultiplyConfirmedCenters() {
    ...
    // Brand change
    if (confirmedCount != EXPECTED_QR_CORNER_COUNT) {
      return false;
    }
  ...
  private FinderPattern[] selectBestPatterns() throws NotFoundException {
    ...
    // Brand change
    if (startSize != EXPECTED_QR_CORNER_COUNT) {
...
//    possibleCenters.sort(moduleComparator);
    ...
    FinderPattern[] bestPatterns = new FinderPattern[EXPECTED_QR_CORNER_COUNT];

    for (int h = 0; h < possibleCenters.size() - 3; h++) {
      FinderPattern fph = possibleCenters.get(h);

      for (int i = h + 1; i < possibleCenters.size() - 2; i++) {
